...
public class FileManager {
	...
	public static List<Account> getAccountList() {
		...
		try {
			reader = new BufferedReader(new FileReader(new File(ACCOUNT_FILE)));
		} catch (FileNotFoundException e) {
			System.out.println("File " + ACCOUNT_FILE + " does not exist");
			return null;
		}
		...
		try {
			...
			try {
				...
				while (line != null && !line.isEmpty()) {
					...
					accountList.add(new Account(accountData[0], accountData[1]));
